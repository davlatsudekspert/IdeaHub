...
<div class="post-card${isDetail ? ' detail' : ''}" id="pc-${p.id}" ${!isDetail ? `onclick="openPost('${p.id}')"` : ''}>
  ...
  <div class="post-body">
    ...
    <div class="post-acts" onclick="event.stopPropagation()">
      ...
      <button class="pa" onclick="copyPostLink('${p.id}')">${IC.share} Ulashish</button>
...
function buildPollHtml(poll, postId, showResults) {
  ...
  return `<div class="poll-card" onclick="event.stopPropagation()" style="background:var(--bg2);border:1px solid var(--border2);border-radius:var(--r-lg);padding:14px;margin-bottom:8px">
    <div style="font-size:13px;font-weight:700;color:var(--tx1);margin-bottom:10px;font-family:'Plus Jakarta Sans',sans-serif">📊 ${esc(poll.question)}</div>
...
/* copyLink(path)/fallbackCopy — umumiy versiyasi core.js'da (postga xos emas,
   problems.js ham shu bittasidan foydalanadi). Bu yerda faqat post-ID uchun qulay o'rash: */
function copyPostLink(id) { copyLink('/?post='+id); }
...
window.voteCmt=voteCmt; window.delCmt=delCmt; window.copyPostLink=copyPostLink;
